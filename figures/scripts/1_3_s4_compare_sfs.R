...
for (h in h_co)  {
  ...
  gg <- ggplot() + 
    ...
    geom_bar(data = df[which(df$Type == "Neutral" & df$ss == sample & df$h_co == h), ],
             mapping = aes(x = Sites, y = as.numeric(Number), fill = Type), 
             position='stack', stat='identity', width = barwidth)+
    geom_bar(data = df[which(df$ss == sample & df$h_co == h & (df$Type == "Deleterious" | df$Type == "Lethal")), ], 
    ...
    scale_fill_manual(values = c("grey78", "black", "grey50"))
    
  
  pdf(paste("figures/", "h_", h, "_compare_sfs.pdf", sep=""),width = 15, height = 8)
  print(gg)
  dev.off()
...
gg = ggplot(lethal_1, aes(fill=ss,
  ...
  scale_fill_manual(values = c("grey78","grey50", "black"))+
...
pdf(paste("figures/", "1percent_sfs.pdf", sep=""),width = 15, height = 6)
print(gg)
dev.off()